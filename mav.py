...
class ModelActivationVisualizer:
        ...
        for _ in range(self.max_new_tokens):
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    print("Model does not support hidden state or attention visualization.")
                    break
    ...
    def _render_visualization(self, generated_ids, next_token_id, mlp_activations, 
                               attn_activations, top_ids, top_probs, logits):
        """
        ...
        max_abs_act = max(
            np.max(np.abs(mlp_activations)), 
            np.max(np.abs(attn_activations))
        )
        ...
        left_panel = Panel(activations_str, title="MLP & Attention Activations", border_style="cyan")
        ...
        top_panel = Panel(highlighted_text, title="Generated Text", border_style="green")
